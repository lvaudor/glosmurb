...
#' @examples
#' data(confluence)
#' save_osmdata("man_made","bridge",confluence)
...
save_osmdata=function(key, value, shape_name, dir_name="data/osmdata"){
  ...
  ngrid=ceiling((sqrt(sf::st_area(shape) %>% as.numeric())/1000)/15)
        ...
        if(nrow(result)>0){
          ...
          if("wikidata" %in% colnames(result)){
                ...
                readr::write_csv(tib_wikidata,
                                 glue::glue("{dir_name}/{shape_name}/wikidata.csv"),append=TRUE)
              }
              result=result %>%
                dplyr::select(osm_id)
